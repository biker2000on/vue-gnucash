...
function CellExternalCopyManager(options) {
  ...
  var _copiedCellStyleLayerKey =
    _options.copiedCellStyleLayerKey || "copy-manager";
  ...
  function _decodeTabularData(_grid, ta) {
    ...
    if (
      clippedRange.length == 1 &&
      clippedRange[0].length == 1 &&
      selectedRange
    ) {
            ...
            for (var x = 0; x < destW; x++) {
              ...
              if (desty < this.maxDestY && destx < this.maxDestX) {
                ...
                if (oneCellToMultiple)
                  this.setDataItemValueForColumn(
                    dt,
                    columns[destx],
                    clippedRange[0][0]
                  );
  ...
  function markCopySelection(ranges) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      for (var j = ranges[i].fromRow; j <= ranges[i].toRow; j++) {
        hash[j] = {};
        for (
          var k = ranges[i].fromCell;
          k <= ranges[i].toCell && k < columns.length;
          k++
        ) {
